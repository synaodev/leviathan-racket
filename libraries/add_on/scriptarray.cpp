...
void RegisterScriptArray(asIScriptEngine* engine, bool defaultArray)
{
	...
	if (defaultArray)
	{
		int r = engine->RegisterDefaultArrayType("std::array<T>"); assert(r >= 0);
		UNUSED_VAR(r);
	}
}
...
CScriptArray::CScriptArray(asITypeInfo* ti, void* buf)
{
	...
	if ((ti->GetSubTypeId() & asTYPEID_MASK_OBJECT) == 0)
	{
		...
		if (length > 0)
			memcpy(At(0), (((asUINT*)buf) + 1), static_cast<size_t>(length)* static_cast<size_t>(elementSize));
	}
	else if (ti->GetSubTypeId() & asTYPEID_OBJHANDLE)
	{
		...
		// its references too
		memset((((asUINT*)buf) + 1), 0, static_cast<size_t>(length)* static_cast<size_t>(elementSize));
	}
	else if (ti->GetSubType()->GetFlags() & asOBJ_REF)
	{
		...
		// Copy the handles into the internal buffer
		if (length > 0)
			memcpy(buffer->data, (((asUINT*)buf) + 1), static_cast<size_t>(length)* static_cast<size_t>(elementSize));
	...
	else
	{
		...
		for (asUINT n = 0; n < length; n++)
		{
			...
			srcObj += 4 + static_cast<size_t>(n)* static_cast<size_t>(ti->GetSubType()->GetSize());
...
void CScriptArray::Reserve(asUINT maxElements)
{
	...
	SArrayBuffer* newBuffer = reinterpret_cast<SArrayBuffer*>(userAlloc(sizeof(SArrayBuffer) - 1 + static_cast<size_t>(elementSize)* static_cast<size_t>(maxElements)));
	if (newBuffer)
	{
	...
	memcpy(newBuffer->data, buffer->data, static_cast<size_t>(buffer->numElements)* static_cast<size_t>(elementSize));
...
void CScriptArray::RemoveRange(asUINT start, asUINT count)
{
	...
	// since we're just copying the pointers to objects and not the actual objects.
	memmove(buffer->data + static_cast<size_t>(start)* static_cast<size_t>(elementSize), buffer->data + (static_cast<size_t>(start) + static_cast<size_t>(count))* static_cast<size_t>(elementSize), (static_cast<size_t>(buffer->numElements) - static_cast<size_t>(start) - static_cast<size_t>(count))* static_cast<size_t>(elementSize));
...
void CScriptArray::Resize(int delta, asUINT at)
{
	...
	{
		// Allocate memory for the buffer
		SArrayBuffer* newBuffer = reinterpret_cast<SArrayBuffer*>(userAlloc(sizeof(SArrayBuffer) - 1 + static_cast<size_t>(elementSize)* (static_cast<size_t>(buffer->numElements) + delta)));
		if (newBuffer)
		{
		...
		// As objects in arrays of objects are not stored inline, it is safe to use memcpy here
		// since we're just copying the pointers to objects and not the actual objects.
		memcpy(newBuffer->data, buffer->data, static_cast<size_t>(at)* static_cast<size_t>(elementSize));
		if (at < buffer->numElements)
			memcpy(newBuffer->data + (static_cast<size_t>(at) + delta)* static_cast<size_t>(elementSize), buffer->data + static_cast<size_t>(at)* static_cast<size_t>(elementSize), (static_cast<size_t>(buffer->numElements) - static_cast<size_t>(at))* static_cast<size_t>(elementSize));
	...
	else if (delta < 0)
	{
		...
		// As objects in arrays of objects are not stored inline, it is safe to use memmove here
		// since we're just copying the pointers to objects and not the actual objects.
		memmove(buffer->data + static_cast<size_t>(at)* static_cast<size_t>(elementSize), buffer->data + (static_cast<size_t>(at) - delta)* static_cast<size_t>(elementSize), (static_cast<size_t>(buffer->numElements) - (static_cast<size_t>(at) - delta))* static_cast<size_t>(elementSize));
	...
	else
	{
		// As objects in arrays of objects are not stored inline, it is safe to use memmove here
		// since we're just copying the pointers to objects and not the actual objects.
		memmove(buffer->data + (static_cast<size_t>(at) + delta)* static_cast<size_t>(elementSize), buffer->data + static_cast<size_t>(at)* static_cast<size_t>(elementSize), (static_cast<size_t>(buffer->numElements) - static_cast<size_t>(at))* static_cast<size_t>(elementSize));
...
const void* CScriptArray::At(asUINT index) const
{
	...
	if ((subTypeId & asTYPEID_MASK_OBJECT) && !(subTypeId & asTYPEID_OBJHANDLE))
		return *(void**)(buffer->data + static_cast<size_t>(elementSize)* static_cast<size_t>(index));
	else
		return buffer->data + static_cast<size_t>(elementSize)* static_cast<size_t>(index);
}
...
{
	*buf = reinterpret_cast<SArrayBuffer*>(userAlloc(sizeof(SArrayBuffer) - 1 + static_cast<size_t>(elementSize)* static_cast<size_t>(numElements)));
...
void CScriptArray::Construct(SArrayBuffer* buf, asUINT start, asUINT end)
{
	...
	else
	{
		...
		void* d = (void*)(buf->data + static_cast<size_t>(start)* static_cast<size_t>(elementSize));
		memset(d, 0, (static_cast<size_t>(end) - static_cast<size_t>(start))* static_cast<size_t>(elementSize));
	}
}
...
{
	return buffer->data + static_cast<size_t>(index)* static_cast<size_t>(elementSize);
}
	...
	else
	{
		if (dst->numElements > 0 && src->numElements > 0)
		{
			...
			else
			{
				...
				memcpy(dst->data, src->data, static_cast<size_t>(count)* static_cast<size_t>(elementSize));
			}
		}
	}
}
